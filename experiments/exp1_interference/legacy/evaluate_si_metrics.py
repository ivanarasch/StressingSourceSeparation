#!/usr/bin/env python
"""
NOTE: this is the original first-pass script (SI-SDR, an SI-SAR-like artifact ratio, and RMSE; mono). It compares each
processed version's separated stems with the separated stems of the *original* (unprocessed) mixture, which is how the
first version of Experiment 2 was scored. It is NOT how Experiment 1 was scored (that uses the true vocal stem; see
src/evaluate_exp1.py, which reuses these metric definitions), and the final Experiment 2
uses src/evaluate.py. Edit ROOT_DIR below before running.
...
ROOT_DIR = "path/to/separated_outputs"   # edit: folder with one sub-folder per song version
