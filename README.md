...
4. **Run**, either from `notebooks/exp2_vocal_effects.ipynb` or in a terminal:
...
Description and results table in [`experiments/exp1_interference/`](experiments/exp1_interference/), code in `src/`, run from `notebooks/exp1_interference.ipynb`. Results (median vocal SI-SDR per pairing) are in `results/exp1/`. The scores were re-checked: recomputing them from the saved outputs reproduces the original write-up exactly.
